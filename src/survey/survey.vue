  ...
  <div class="max-w-3xl mx-auto py-32">
    <div class="bg-white shadow-xl rounded-xl p-8 space-y-6 border">
      <h1 class="text-3xl font-extrabold text-gray-800 mb-4">
        Personalize your Tech Trends Report
      </h1>
      <form @submit.prevent="submitForm" class="space-y-8">
        ...
        <div>
          <label class="block text-lg font-semibold text-gray-700 mb-2">
            ...
            <span class="text-sm text-gray-500"
              >(We will suggest technologies based on your planning
              horizon)</span
            >
          </label>
          <select
        ...
        <div>
          ...
          >
            <div class="flex items-center mb-1">
              <h3
                :class="
                  valueColors[category] + ' font-large font-extrabold mr-2'
                "
              >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <label v-for="subvalue in subvalues[category]" :key="subvalue">
                <input
                  type="checkbox"
                  v-model="selectedSubvalues[category]"
                  :value="subvalue"
                />
                {{ subvalue }}
              </label>
          ...
          <p
            v-if="
              showError && Object.values(selectedSubvalues).flat().length === 0
            "
        ...
        <!-- Warning -->
        <p
          v-if="Object.values(selectedSubvalues).flat().length > 3"
          class="text-red-600 text-sm"
        >
          Please select no more than 3 values.
        </p>
      </form>
    </div>
  </div>
...
const subvalues = {
  ...
  Justice: ["Inclusivity", "Equality", "Integrity"],
  Humanity: [
    "Respect",
    "Health, well-being",
    "Safety",
...
function submitForm() {
  if (
    !size.value ||
    Object.values(selectedSubvalues).flat().length === 0 ||
    Object.values(selectedSubvalues).flat().length > 3
  ) {
  ...
  trends.forEach((trend) => {
    const isReadinessMatch = (() => {
      ...
      if (readiness === "plan")
        return ["act", "plan"].includes(trend.readiness);
      if (readiness === "watch")
        return ["plan", "watch"].includes(trend.readiness);
      return false;
    })();

    if (isReadinessMatch) {
      const matchingSubvalues = trend.subvalues.filter((subvalue) =>
        selectedValues.includes(subvalue),
      );
  ...
  router.push({
    ...
    query: {
      size: size.value,
      values: Object.keys(selectedSubvalues)
        .filter((key) => selectedSubvalues[key].length)
        .join(", "),
      readiness,
